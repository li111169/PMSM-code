...
rep_element=vector_info.coord_value(index,[1 2]);
[~,rep_index]=Check_Repetition(rep_element);
...
xlim([-2.5 2.5]);
ylim([-2.5 2.5]);
text(coord_value(1,vector_info.flag==0)'+0.1,coord_value(2,vector_info.flag==0)',dec2base(bin2dec(num2str(vector_info.abcuvw(vector_info.flag==0,:))),8));

text(vector_info.coord_value(rep_index,1)+0.1,vector_info.coord_value(rep_index,2),dec2base(bin2dec(num2str(vector_info.abcuvw(vector_info.coord_value,:))),8));
